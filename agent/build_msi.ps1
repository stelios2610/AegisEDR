...
Write-Host "=== AegisEDR MSI Builder ===" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "  SKIP: Asset generation (pip install cairosvg Pillow)" -ForegroundColor Yellow
}

# Step 2: Build AegisEDR-Agent.exe
Write-Host ""
...
if ($LASTEXITCODE -ne 0) { Write-Host "FAILED: PyInstaller agent" -ForegroundColor Red; exit 1 }
...
python -m PyInstaller @trayArgs
if ($LASTEXITCODE -ne 0) {
    Write-Host "  WARN: Tray build failed (pip install pystray Pillow)" -ForegroundColor Yellow
} else {
    Write-Host "  OK: AegisEDR-Tray.exe" -ForegroundColor Green
}

# Step 3b: Build AegisEDR.exe (dashboard UI)
Write-Host ""
Write-Host "[3b] Building AegisEDR.exe (dashboard)..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "  WARN: App build failed (pip install customtkinter pillow)" -ForegroundColor Yellow
} else {
...
if (-not $wix) {
    ...
    Write-Host "  OK: WiX v4 installed" -ForegroundColor Green
} else {
    Write-Host "  OK: WiX $(wix --version)" -ForegroundColor Green
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "FAILED: WiX build" -ForegroundColor Red
    exit 1
}
...
Write-Host "File: $msiOutput" -ForegroundColor Green
Write-Host "Size: $size MB" -ForegroundColor Green
Write-Host "Console URL baked in: $ConsoleUrl" -ForegroundColor Green
Write-Host ""

# Copy to downloads folder for web serving
$DownloadsDir = Join-Path $Root "downloads"
New-Item -ItemType Directory -Force -Path $DownloadsDir | Out-Null
Copy-Item $msiOutput $DownloadsDir -Force
Write-Host "Copied to downloads/ for console serving." -ForegroundColor Green
